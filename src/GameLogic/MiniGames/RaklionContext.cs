...
/// <summary>
/// The context of a raklion boss game.
/// </summary>
/// <remarks>
/// Raklion is a boss event where players enter the Raklion Boss map
/// and fight through monsters to reach the Selupan boss.
/// Players earn score by killing monsters (boss kill grants bonus).
/// Duration is 30 minutes, up to 15 players.
/// Rewards are experience, money and jewel drops based on ranking.
/// </remarks>
public sealed class RaklionContext : MiniGameContext
{
    /// <summary>
    /// Selupan boss monster number.
    /// </summary>
    ...
    protected override async void OnDestructibleDied(object? sender, DeathInformation e)
    {
        try
        {
            ...
            if (sender is Destructible { Definition.Number: SelupanNumber })
            {
