...
/// </summary>
public readonly ref struct OAuthLoginRef
{
    private readonly Span<byte> _data;

    /// <summary>
    /// Initializes a new instance of the <see cref="OAuthLoginRef"/> struct.
    /// </summary>
    /// <param name="data">The underlying data.</param>
    public OAuthLoginRef(Span<byte> data)
        : this(data, true)
    {
    }
    ...
    /// <param name="data">The underlying data.</param>
    /// <param name="initialize">If set to <c>true</c>, the header data is automatically initialized and written to the underlying span.</param>
    private OAuthLoginRef(Span<byte> data, bool initialize)
    {
        this._data = data;
        if (initialize)
        {
    ...
    /// <summary>
    /// Gets or sets 0=Google, 1=Facebook, 2=Apple
    /// </summary>
    ...
    /// <summary>
    /// Performs an implicit conversion from a Span of bytes to a <see cref="OAuthLogin"/>.
    /// </summary>
    /// <param name="packet">The packet as span.</param>
    /// <returns>The packet as struct.</returns>
    public static implicit operator OAuthLoginRef(Span<byte> packet) => new (packet, false);

    /// <summary>
    /// Performs an implicit conversion from <see cref="OAuthLogin"/> to a Span of bytes.
    /// </summary>
    /// <param name="packet">The packet as struct.</param>
    /// <returns>The packet as byte span.</returns>
    public static implicit operator Span<byte>(OAuthLoginRef packet) => packet._data; 

    /// <summary>
    /// Calculates the size of the packet for the specified field content.
    /// </summary>
    /// <param name="content">The content of the variable 'Token' field from which the size will be calculated.</param>
    public static int GetRequiredSize(string content) => System.Text.Encoding.UTF8.GetByteCount(content) + 1 + 5;

    /// <summary>
    /// Calculates the size of the packet for the specified field content.
    /// </summary>
    /// <param name="contentLength">The content length in bytes of the variable 'Token' field from which the size will be calculated.</param>
